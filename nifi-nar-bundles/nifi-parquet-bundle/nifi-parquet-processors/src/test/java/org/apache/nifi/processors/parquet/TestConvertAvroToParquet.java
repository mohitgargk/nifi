...
public class TestConvertAvroToParquet {
    ...
    public void test_Meta_Info() throws Exception {
        ...
        FileOutputStream fos = new FileOutputStream("test.parquet");
        ...
        metaData = ParquetFileReader.readFooter(conf, new Path("test.parquet"), NO_FILTER);
    ...
    public void test_Data() throws Exception {
        ...
        Configuration conf = new Configuration();
        FileSystem fs = FileSystem.getLocal(conf);
        ParquetReader<Group> reader = ParquetReader.builder(new GroupReadSupport(), new Path("test.parquet"))
                        .withConf(conf)
                        .build();
